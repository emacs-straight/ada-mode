...
package body WisiToken.Parse.LR.McKenzie_Recover.Ada is
   ...
   is
      ...
      End_Name_Token : constant Recover_Token :=
        (if Config.In_Parse_Action_Status.End_Name > 0
         then Recover_Stacks.Peek
           (Config.Stack, Config.In_Parse_Action_Token_Count - Config.In_Parse_Action_Status.End_Name + 1).Token
         else Invalid_Recover_Token);

      Handled_Statements_Peek : constant SAL.Peek_Type :=
        --  The production is either:
        --  ... handled_sequence_of_statements END name ';'
        --  ... handled_sequence_of_statements END ';'
        (if Config.In_Parse_Action_Status.End_Name > 0 then 4 else 3);
   begin
      ...
      when Match_Names_Error =>
         ...
         declare
            pragma Assert (End_Name_Token /= Invalid_Recover_Token); -- Otherwise we'd have missing or extra
            ...
            if Matching_Name_Index = Config.Stack.Depth then
               ...
               begin
                  ...
                  when block_statement_ID =>
                     Push_Back_Check
                       (Super, Shared_Parser, New_Config,
                        (+SEMICOLON_ID, +IDENTIFIER_ID, +END_ID),
                  ...
                  when entry_body_ID =>
                     Push_Back_Check
                       (Super, Shared_Parser, New_Config,
                        (+SEMICOLON_ID, +IDENTIFIER_ID, +END_ID, +handled_sequence_of_statements_ID),
                        Push_Back_Undo_Reduce => True);
                     Insert (Super, Shared_Parser, New_Config, +BEGIN_ID);
                  ...
                  when loop_statement_ID =>
                     Push_Back_Check
                       (Super, Shared_Parser, New_Config,
                        (+SEMICOLON_ID, +IDENTIFIER_ID, +LOOP_ID, +END_ID),
                        Push_Back_Undo_Reduce => True);
                  ...
                  when package_body_ID =>
                     Push_Back_Check
                       (Super, Shared_Parser, New_Config,
                        (+SEMICOLON_ID, +simple_name_ID, +END_ID),
                        Push_Back_Undo_Reduce => True);
                     ...
                     if Tree.Element_ID (New_Config.Stack.Peek (1).Token) = +non_empty_declarative_part_ID then
                        Push_Back_Check
                          (Super, Shared_Parser, New_Config,
                           (1 => +non_empty_declarative_part_ID),
                           Push_Back_Undo_Reduce => True);
                     end if;
                  ...
                  when package_specification_ID =>
                     ...
                     Push_Back_Check
                       (Super, Shared_Parser, New_Config,
                        (+simple_name_ID, +END_ID, +basic_declarative_item_pragma_list_ID),
                        Push_Back_Undo_Reduce => True);
                     if Tree.Element_ID (New_Config.Stack.Peek (1).Token) = +PRIVATE_ID then
                        Push_Back_Check
                          (Super, Shared_Parser, New_Config,
                           (+PRIVATE_ID, +basic_declarative_item_pragma_list_ID),
                           Push_Back_Undo_Reduce => True);
                     end if;
                  ...
                  when subprogram_body_ID =>
                     Push_Back_Check
                       (Super, Shared_Parser, New_Config,
                        (+SEMICOLON_ID, +name_ID, +END_ID, +handled_sequence_of_statements_ID, +BEGIN_ID),
                        Push_Back_Undo_Reduce => True);
                     ...
                     (+SEMICOLON_ID,
                      (case To_Token_Enum (Tree.Element_ID (Config.Error_Token)) is
                       when package_body_ID | package_specification_ID =>
                          +simple_name_ID,
                       when protected_type_declaration_ID | single_protected_declaration_ID =>
                          +protected_definition_ID,
                       when subprogram_body_ID =>
                          +name_ID,
                       when others =>  +IDENTIFIER_ID)),
                     Push_Back_Undo_Reduce => True);
      ...
      when Missing_Name_Error =>
         ...
         --  Check if 'exception' is present in the handled_sequence_of_statements
         if Config.Stack.Depth < Handled_Statements_Peek or else
           Config.Stack.Peek (Handled_Statements_Peek).Token.Virtual or else
           not Is_Nonterminal
             (Tree.Element_ID (Config.Stack.Peek (Handled_Statements_Peek).Token), Tree.Lexer.Descriptor.all)
         then
            return;
         end if;

         if Invalid_Node_Access = Tree.Find_Child
           (Config.Stack.Peek (Handled_Statements_Peek).Token.Node, +EXCEPTION_ID)
         then
            ...
            begin
               ...
               when block_statement_ID | package_body_ID | subprogram_body_ID | task_body_ID =>
                  ...
                  Push_Back_Check
                    (Super, Shared_Parser, New_Config, (+SEMICOLON_ID, +END_ID),
                     Push_Back_Undo_Reduce => True);
               ...
               when package_specification_ID =>
                  Push_Back_Check
                    (Super, Shared_Parser, New_Config,
                     ...
                     Push_Back_Undo_Reduce => True);
                  Undo_Reduce_Check
                    (Super, Shared_Parser, New_Config, +basic_declarative_item_pragma_list_ID);
            ...
            begin
               ...
               Push_Back_Check
                 (Super, Shared_Parser, New_Config,
                  (+SEMICOLON_ID, +END_ID, +handled_sequence_of_statements_ID),
                  Push_Back_Undo_Reduce => True);
         ...
         begin
            ...
            if New_Config.Stack.Peek (First_Begin_I).Token.Virtual then
               ...
               when COLON_ID =>
                  ...
                  Push_Back_Check
                    (Super, Shared_Parser, New_Config, (+COLON_ID, +IDENTIFIER_ID),
                     Push_Back_Undo_Reduce => True);
               ...
               if Undo_Reduce_Valid (Super, Shared_Parser, New_Config) then
                  Undo_Reduce_Check (Super, Shared_Parser, New_Config, +statement_list_ID);
               elsif Push_Back_Valid (Super, Shared_Parser, New_Config, Push_Back_Undo_Reduce => True) then
                  Push_Back_Check (Super, Shared_Parser, New_Config, +statement_list_ID,
                                   Push_Back_Undo_Reduce => True);
               else
                  raise Invalid_Case;
               end if;
   ...
   begin
      if (Tree.Element_ID (Config.Error_Token) = +COLON_ID and
            (Tree.Element_ID (Config.Stack.Peek.Token) /= Invalid_Token_ID and then
               -Tree.Element_ID (Config.Stack.Peek.Token) in IDENTIFIER_ID)) and then
      ...
      then
               ...
               begin
                  if Push_Back_Valid (Super, Shared_Parser, New_Config_1, Push_Back_Undo_Reduce => True) then
                     ...
                     if Undo_Reduce_Valid (Super, Shared_Parser, New_Config_2) then
                        if Tree.Element_ID (New_Config_2.Stack.Peek.Token) = +non_empty_declarative_part_ID then
                           Undo_Reduce_Check (Super, Shared_Parser, New_Config_2, +non_empty_declarative_part_ID);
                        end if;
                     end if;
         ...
         if To_Token_Enum (Tree.Element_ID (Config.Stack.Peek.Token)) in IDENTIFIER_ID and
         ...
         then
            ...
            begin
               ...
               if To_Token_Enum (Tree.Element_ID (New_Config_1.Stack.Peek (3).Token)) in
                 DECLARE_ID | non_empty_declarative_part_ID
               then
                  ...
                  begin
                     ...
                     when handled_sequence_of_statements_ID |
                       sequence_of_statements_ID => --  see test/ada_mode-recover_partial_24.adb
                        ...
                        if To_Token_Enum (Tree.Element_ID (New_Config_2.Stack.Peek.Token)) = COLON_ID then
                           Push_Back_Check
                             (Super, Shared_Parser, New_Config_2,
                              (+COLON_ID, +IDENTIFIER_ID),
                              Push_Back_Undo_Reduce => True);
                        end if;
      ...
      then
         --  We've encountered a token after 'end' when expecting a different
         --  token. See test/ada_mode-recover_08, _17, _20, _24, _26, _39.adb.
         --
         --  There are several cases:
         --
         ...
         declare
            ...
            begin
               ...
               when Shift =>
                  ...
                  if End_ID_Actions (End_ID_Actions.First_Index).ID = +SEMICOLON_ID then
                     --  case c; no token following 'end'.
                     Insert (Super, Shared_Parser, New_Config, (+END_ID, +SEMICOLON_ID));

                     Local_Config_Heap.Add (New_Config);
                     if Trace_McKenzie > Detail then
                        Put ("Language_Fixes wrong end keyword c", New_Config);
                     end if;
               ...
               when Reduce =>
                  --  Assume reduce to a name; case c: ada_mode-recover_39.adb.
                  Insert (Super, Shared_Parser, New_Config, (+END_ID, +IDENTIFIER_ID, +SEMICOLON_ID));
         ...
         begin
            if To_Token_Enum (Tree.Element_ID (Config.Error_Token)) /= IDENTIFIER_ID then
               ...
               begin
                  if not Found then
                     ...
                     --  simpler to delete 'end <keyword> ;' and let minimal_complete handle it.
                     ...
                     begin
                        ...
                        if +SEMICOLON_ID = Tree.ID (Peek_Sequential_Terminal (Peek_State)) then
                     ...
                     Local_Config_Heap.Add (New_Config);
                     if Trace_McKenzie > Detail then
                        Put ("Language_Fixes wrong keyword d", New_Config);
                     end if;
            ...
            --  Now try case b.
            if Tree.Element_ID (Config.Error_Token) = +IDENTIFIER_ID then
               declare
                  New_Config : aliased Configuration := Config;
      ...
      then
         ...
         if Push_Back_Valid (Super, Shared_Parser, Config, Push_Back_Undo_Reduce => True) then
            ...
            begin
               ...
               if Tree.Element_ID (New_Config.Stack.Peek.Token) = +non_empty_declarative_part_ID then
                  if Undo_Reduce_Valid (Super, Shared_Parser, New_Config) then
                     Undo_Reduce_Check (Super, Shared_Parser, New_Config, +non_empty_declarative_part_ID);
                  else
                     raise Invalid_Case;
                  end if;
               end if;
   ...
   begin
      ...
      when END_ID =>
         ...
         when LOOP_ID =>
            if Tree.Element_ID (Config.Stack.Peek (2).Token) /= Invalid_Token_ID and then
              To_Token_Enum (Tree.Element_ID (Config.Stack.Peek (2).Token)) = END_ID
            then
